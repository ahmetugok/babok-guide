...
export function BusinessRulesTab() {
  ...
  const openBRModal   = useUIStore((s) => s.openBRModal);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => openBRModal(null)} className="bg-blue-600/80 hover:bg-blue-500 text-white px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors shadow-lg shadow-black/20"><Plus className="w-4 h-4" />Kural Ekle</button>
      </div>
      {(activeProject.businessRules || []).length === 0 ? (
        <div className="text-center py-16 text-slate-400"><BookOpen className="w-10 h-10 mx-auto mb-3 opacity-30" /><p>Henuz is kurali eklenmemis.</p><button onClick={() => openBRModal(null)} className="mt-3 text-xs text-blue-400 hover:text-blue-300 transition-colors">+ Kural Ekle</button></div>
      ) : (
                  ...
                  <tr key={r.id} className={`hover:bg-white/5 transition-colors ${r.status === 'Gecersiz' ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openBRModal(r.id)} className="p-1 hover:bg-white/10 rounded text-slate-400 hover:text-blue-600 transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
