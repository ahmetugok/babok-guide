    ...
    <div className="space-y-4">
      ...
      {(assumptions || []).length === 0 ? (
        <div className="text-center py-20 glass-card p-8">
          ...
          <button onClick={() => openModal('assumption')} className="mt-4 text-xs text-amber-400 hover:text-amber-300 transition-colors">+ Varsayim Ekle</button>
        </div>
      ) : (
          ...
          {(assumptions || []).map(a => (
            <div key={a.id} className={`bg-white/5 rounded-xl border p-4 shadow-lg shadow-black/20 flex items-start gap-4 ${a.validationStatus === 'Curutuldu' ? 'border-l-4 border-l-rose-400' : 'border-l-4 border-l-amber-400/40'}`}>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => openModal('assumption', { editingId: a.id })} className="p-1.5 hover:bg-white/10 rounded-md text-slate-400 hover:text-blue-600 transition-colors"><Pencil className="w-4 h-4" /></button>
                <button onClick={() => { if (window.confirm('Varsayımı silmek istiyor musunuz?')) deleteAssumption(a.id); }} className="p-1.5 hover:bg-rose-500/10 rounded-md text-slate-400 hover:text-rose-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
